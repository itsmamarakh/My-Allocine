...
requests["Find the name of all reviewers who have rated 7 or more stars to their rating order by reviews rev_name (it might have duplicated names :-))"] = "SELECT rev_name FROM reviews, movies_ratings_reviews WHERE movies_ratings_reviews.rev_id = reviews.id AND movies_ratings_reviews.rev_stars>=7 ORDER BY reviews.rev_name;"
